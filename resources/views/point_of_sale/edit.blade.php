<x-app-layout>
    ...
    <form class="space-y-6" action="{{ route('pos.update', $point_of_sale->id) }}" method="POST" enctype="multipart/form-data" autocomplete="false">
        @method('PUT')    
        {{ csrf_field() }}
        <div class="space-y-4">
            ...
            <div>
                <div class="w-1/3">
                    ...
                    <div class="relative flex w-full">
                        ...
                        <option value="{{ $bchannel->id }}" {{ $bchannel->id == $point_of_sale->booking_channel_id ? 'selected' : ''}}>{{ $bchannel->name }}</option>
                        @endforeach
                        </select>
            ...
            <div>
                <div class="w-1/3">
                    ...
                    <div class="relative flex w-full">
                        ...
                        <option value="{{ $cname->id }}" {{ $cname->id == $point_of_sale->company_id ? 'selected' : ''}}>{{ $cname->name }}</option>
                        @endforeach
                        </select>
                        <div class="absolute inset-y-0 right-0 flex items-center px-2 text-blue-400 pointer-events-none">
                        <svg class="w-6 h-6 fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
